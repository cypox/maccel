...
#include "macc.h"
	...
	data_t output_4d_hw[BATCH][WEIGHT_NUM][OUTPUT_W][OUTPUT_H];
	data_t output_4d_sw[BATCH][WEIGHT_NUM][OUTPUT_W][OUTPUT_H];
#endif
...
int main(int argc, char** argv)
{
	...
	macc_4d(input_4d, weight_4d, output_4d_hw);
	macc_4d_ref(input_4d, weight_4d, output_4d_sw);

	for ( uint i = 0 ; i < WEIGHT_NUM ; ++ i )
		for ( uint j = 0 ; j < OUTPUT_H ; ++ j)
			for ( uint k = 0 ; k < OUTPUT_W ; ++ k)
				if ( output_4d_sw[0][i][j][k] != output_4d_hw[0][i][j][k] )
				{
					std::cout << "Outputs does not match!" << std::endl;
					return -1;
				}
