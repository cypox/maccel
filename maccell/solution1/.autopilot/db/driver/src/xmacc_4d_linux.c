...
typedef struct {
    ...
    XMacc_4d_uio_map maps[ MAX_UIO_MAPS ];
} XMacc_4d_uio_info;

/***************** Variable Definitions **************************************/
static XMacc_4d_uio_info uio_info;
...
static int uio_info_read_name(XMacc_4d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMacc_4d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMacc_4d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMacc_4d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMacc_4d_Initialize(XMacc_4d *InstancePtr, const char* InstanceName) {
	XMacc_4d_uio_info *InfoPtr = &uio_info;
